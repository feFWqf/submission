...
class CMFENet():
    ...
    def do_train(self, model, dataloader):
        ...
        logger.info(f'The fusion module (CMFENet) has {count_parameters(model)} parameters.')
...
# 
        model_params_other = [
          p for n, p in list(model.Model.named_parameters())
        ...
        while True: 
            ...
            with tqdm(dataloader['train']) as td:
                for batch_idx, batch_data in enumerate(td, 1):
                    ...
                    loss_attra2_text = -(criterion_attra(outputs['pp_text_m'], outputs['zz_text']).mean() +
                                          criterion_attra(outputs['pp_text'], outputs['zz_text_m']).mean()) * 0.5
                    loss_attra2_audio = -(criterion_attra(outputs['pp_audio_m'], outputs['zz_audio']).mean() +
                                          criterion_attra(outputs['pp_audio'], outputs['zz_audio_m']).mean()) * 0.5
                    loss_attra2_video = -(criterion_attra(outputs['pp_video_m'], outputs['zz_video']).mean() +
                                          criterion_attra(outputs['pp_video'], outputs['zz_video_m']).mean()) * 0.5
                    #loss_attra1 =  loss_attra_text + loss_attra_audio + loss_attra_video 
                    loss_attra =  loss_attra2_text + loss_attra2_audio + loss_attra2_video 
                   ...
                   # loss_recon_text2 = criterion_recon(outputs['p_text'], outputs['text_for_recon'], mask)
                   
                    mask = audio_mask - audio_missing_mask#torch.Size([32, 400])
                    loss_recon_audio = criterion_recon(outputs['audio_recon'], audio, mask)
                   # loss_recon_audio2 = criterion_recon(outputs['p_audio'], audio, mask)

                    mask = vision_mask - vision_missing_mask
                    loss_recon_video = criterion_recon(outputs['video_recon'], vision, mask)
                    #l#oss_recon_video2 = criterion_recon(outputs['p_video'], vision, mask)
                    loss_recon= loss_recon_text + loss_recon_audio + loss_recon_video
         
                    loss = loss_pred_m  +loss_pred+loss_recon*0.3+ loss_attra*0.5
