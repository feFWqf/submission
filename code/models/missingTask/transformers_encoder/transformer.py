...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, embed_dim, num_heads=4, attn_dropout=0.1, relu_dropout=0.1, res_dropout=0.1,
        ...
        self.layer_norms = nn.ModuleList([LayerNorm(self.embed_dim) for _ in range(2)]) #定义两个层规范化模块。
    ...
    def forward(self, x, x_k=None, x_v=None):
        ...
        if x_k is None and x_v is None: #定义的一个注意力机制，根据输入是自注意力还是交叉注意力
...
#在注意力机制中，这确保了解码器在预测序列的某个元素时不会查看该元素之后的信息。
def buffered_future_mask(tensor, tensor2=None):
    dim1 = dim2 = tensor.size(0)
    if tensor2 is not None:
        dim2 = tensor2.size(0)
    future_mask = torch.triu(fill_with_neg_inf(torch.ones(dim1, dim2)), 1+abs(dim2-dim1)) #三角部分的矩阵
    if tensor.is_cuda:
